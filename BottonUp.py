...
class BUParser(Parser):
    ...
    def analyce_word(self, word: str) -> bool:
        ...
        i = 0
        while True:
            if(self.action[stack[-1]][word[i]] != float('inf') and self.action[stack[-1]][word[i]][0] == 's'):
                element = self.action[stack[-1]][word[i]][1]
                stack.append(element)
                i += 1
            elif(self.action[stack[-1]][word[i]] != float('inf') and self.action[stack[-1]][word[i]][0] == 'r'):
            ...
            elif(self.action[stack[-1]][word[i]] != float('inf') and self.action[stack[-1]][word[i]][0] == 'a'):
                return True
            else:
                return False