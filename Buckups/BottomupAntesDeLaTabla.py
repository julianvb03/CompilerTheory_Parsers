...
class BUParser(Parser):
    ...
    def predictive_parsing_table(self) -> list:       
        ...
        actual = initial
        Action[index] = { x:0 for x in self.grammar.Terminals.union('$') }
        Goto[index] = {x:0 for x in self.grammar.Non_Terminals}
        #mapping.append((0,initial))
        ...
        while actual not in visiteds:
            ...
            for symbol in self.symbol_in_state(actual):
                ...
                if new_state not in states and len(new_state) > 0:
                    ...
                    Action[index] = { x:0 for x in self.grammar.Terminals.union('$') }
                    Goto[index] = {x:0 for x in self.grammar.Non_Terminals}
                    #mapping.append((index,frozenset(new_state)))
                if len(new_state) > 0:
                    states.add(frozenset(new_state))

                '''if symbol in self.grammar.Non_Terminals:
                    if Goto[index][symbol] == 0:
                       Goto[index][symbol] = index + 1
                    else:
                        raise SyntaxError'''
